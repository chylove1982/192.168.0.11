...
<script>
import pendingApi from '@/api/accounting-manage/pending'
...
export default {
  ...
  methods: {
    ...
    // 删除科室
    delfun(id) {
      pendingApi.delById(id).then(response => {
    ...
    savefun() {
      ...
      pendingApi.addSec(this.secform).then(response => {
        console.log('response', response)
        const { code, msg } = response
        if (Object.is(code, 1)) {
          this.$message.success('添加成功')
    ...
    getList(data) {
      ...
      pendingApi.getList(this.listQuery).then(response => {
        console.log('response', response)
