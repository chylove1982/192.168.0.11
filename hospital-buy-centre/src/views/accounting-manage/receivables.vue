<template>
  <div class="app-container">
    ...
    >
      ...
      <form-com :formface="secformface" :form="secform" :config="secformConfig" />
...
<script>
import receivablesApi from '@/api/accounting-manage/receivables'
...
export default {
  ...
  methods: {
    ...
    // 删除科室
    delfun(id) {
      receivablesApi.delById(id).then(response => {
    ...
    savefun() {
      ...
      receivablesApi.addSec(this.secform).then(response => {
        console.log('response', response)
        const { code, msg } = response
        if (Object.is(code, 1)) {
          this.$message.success('添加成功')
    ...
    getList(data) {
      ...
      receivablesApi.getList(this.listQuery).then(response => {
        console.log('response', response)
        const { code, result, count, msg } = response
        if (Object.is(code, 1)) {
          this.secList = result
          this.total = count
        } else {
