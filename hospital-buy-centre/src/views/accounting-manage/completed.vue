...
<script>
import completedApi from '@/api/accounting-manage/completed'
...
export default {
  ...
  methods: {
    ...
    // 删除科室
    delfun(id) {
      completedApi.delById(id).then(response => {
    ...
    savefun() {
      ...
      completedApi.addSec(this.secform).then(response => {
        console.log('response', response)
        const { code, msg } = response
        if (Object.is(code, 1)) {
          this.$message.success('添加成功')
    ...
    getList(data) {
      ...
      completedApi.getList(this.listQuery).then(response => {
        console.log('response', response)
