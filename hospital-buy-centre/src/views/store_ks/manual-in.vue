...
<script>
  // import manualInApi from '@/api/store/manual-in'
  export default {
    data() {
      return {
        form: {}, // 获取的返回数据
    ...
    methods: {
      ...
      getInfo() {
        ...
        manualInApi.getInfo({ 'spduserinfoid': spduserinfoid }).then(response => {
      ...
      // 修改信息
      updateInfo() {
        manualInApi.updateInfo(this.form).then(response => {
