<template>
  <div class="app-container">
    ...
    >
      ...
      <form-com :formface="basicInfoQsface" :config="basicInfoQsConfig" :form="basicInfo.heard" />
...
export default {
  data() {
    return {
      ...
      config: { // 前端定义
        ...
        tableBtns: [{ name: '详情', type: 'add', apiurl: 'api/SPDApplyOrder/GetSPDApplyOrderMasterSaveByID' }]
      }, // 控制列表加载动画化
      listQuery: { // 查询条件
        PageSize: 5,
        PageIndex: 1,
  ...
  methods: {
    ...
    getList() {
      const tparam = 'SPDDistributorordercopewithdetailed'// 修改此处参数；在配置表中的对象名
      var dt = this.gettable(tparam, this.listQuery)
      console.log('dt', dt)
    ...
    controlFun(e, r) {
      ...
      this.basicInfoShface = this.gettableheaders('GetSPDApplyOrderMasterSaveByIDSh')
      this.basicInfoQsface = this.gettableheaders('GetSPDApplyOrderMasterSaveByIDQs')
