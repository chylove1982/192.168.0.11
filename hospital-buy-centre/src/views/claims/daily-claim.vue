<template>
  <div class="app-container">
    ...
    >
      ...
      <!--签收信息-->
      <h3 class="title_line">签收信息</h3>
      <form-com :formface="basicInfoQsface" :config="basicInfoQsConfig" :form="basicInfoQs" />

      <!--领用产品明细-->
      <h3 class="title_line">领用产品明细</h3>
      <form-com :formface="basicInfoMxface" :config="basicInfoMxConfig" :form="basicInfoMx" />
    </el-dialog>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      config: { // 前端定义
        ...
        tableBtns: [{ name: '请领详情', type: 'add', apiurl: 'api/SPDApplyOrder/GetSPDApplyOrderMasterSaveByID' }]
      }, // 控制列表加载动画化
      listQuery: { // 查询条件
        PageSize: 5,
        PageIndex: 1,
  ...
  methods: {
    ...
    controlFun(e, r) {
      ...
      this.basicInfoSh = this.getformdata(this.config.tableBtns[e], r)
      this.basicInfoShface = this.gettableheaders('GetSPDApplyOrderMasterSaveByID')

      this.basicInfoQs = this.getformdata(this.config.tableBtns[e], r)
      this.basicInfoQsface = this.gettableheaders('GetSPDApplyOrderMasterSaveByID')

      this.basicInfoMx = this.getformdata(this.config.tableBtns[e], r)
      this.basicInfoMxface = this.gettableheaders('GetSPDApplyOrderMasterSaveByID')
