...
<script>
import scanCodeInApi from '@/api/store/scanCode-in'
  export default {
    data() {
      return {
        form: {}, // 获取的返回数据
    ...
    methods: {
      ...
      getInfo() {
        ...
        scanCodeInApi.getInfo({ 'spduserinfoid': spduserinfoid }).then(response => {
      ...
      // 修改信息
      updateInfo() {
        scanCodeInApi.updateInfo(this.form).then(response => {
