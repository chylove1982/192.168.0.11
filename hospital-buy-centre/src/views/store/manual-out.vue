...
<script>
  import manualOutApi from '@/api/store/manual-out'
  export default {
    data() {
      return {
        form: {}, // 获取的返回数据
    ...
    methods: {
      ...
      getInfo() {
        ...
        manualOutApi.getInfo({ 'spduserinfoid': spduserinfoid }).then(response => {
      ...
      // 修改信息
      updateInfo() {
        manualOutApi.updateInfo(this.form).then(response => {
