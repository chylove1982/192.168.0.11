        ...
        <template slot-scope="{row}">
          ...
          <div v-else-if="item.chinese=== '操作'">
            <div v-for="(btn,index) in config.tableBtns">
              <el-button class="btn" v-if="row.state!== undefined?btn.state===row.state:true"  type="primary" size="mini" plain @click="controlBtn(index,row)">{{ btn.name }}</el-button>
            </div>
          </div>
          <!-- 如果是普通的 -->
...
export default {
  ...
  props: {
   ...
   var  dt=this.gettable(this.tableconfig,this.listQuery)
     this.tableface= dt.headers
     this.newTable = dt.datalist
  },
  data() {
    return {
      tableface:[],
  ...
  watch:{
    table(){
     ...
     var  dt=this.gettable(tableconfig,listQuery)
     this.tableface= dt.headers
     this.newTable = dt.datalist
    }
  },
