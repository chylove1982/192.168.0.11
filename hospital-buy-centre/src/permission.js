...
import { getToken } from '@/utils/auth' // get token Form cookie
    ...
    if (to.path === '/login') {
      ...
      if (hasmenu) {
        ...
        try {
          ...
          const { userDetails } = await store.dispatch('user/getInfo')
          var menu = []
          userDetails[0].subMenu.forEach(item => {
            menu.push(item.id)
            if (item.subMenu && item.subMenu.length > 0) {
              const subIds = item.subMenu.map(subItem => subItem.id)
